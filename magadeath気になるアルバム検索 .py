...
selected_album = st.selectbox('アルバムの評価サイト', filtered_df['Title'])
if selected_album:
    url = filtered_df[filtered_df['Title'] == selected_album]['Title_URL'].values[0]
    st.markdown(f"[{selected_album}のMusicBrainzパージ]({url})", unsafe_allow_html=True)
...
st.subheader(f"{sort_key} によるランキング(上位10件)")
